...
class MovieLikeButton extends React.Component {
    ...
    render() {
        ...
        if (isMovieLikedByUser({ currentUserLikedMovies, imdbID })) {
            return (<button className="like-btn btn btn-primary" onClick={this.unLikeMovie}>Unlike</button>);
        } else {
            return (<button className="like-btn btn btn-primary" onClick={this.postLikedMovie}>Like</button>);
        }
    }
